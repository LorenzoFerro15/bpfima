...
/**
 * bpfima_measurement_extend - Extend IMA measurement list
...
/**
 * bpfima_tpm_is_available - Check if TPM is available
...
/**
 * bpfima_container_get_or_create - Create or get existing container tracking
...
/**
 * bpfima_container_exists - Check if container exists
 * @container_id: Container/namespace identifier
 *
 ...
 */
extern int bpfima_container_get_measurement_count(const char *container_id) __ksym;

/**
 * bpfima_container_get_count - Get total number of tracked containers
 * Returns: Number of containers, negative on error
 */
extern int bpfima_container_get_count(void) __ksym;

/**
 * bpfima_container_get_leaf_hash - Get the leaf hash for a container
 * @container_id: Container/namespace identifier
 ...
 * Returns: 0 on success, negative error code on failure
 */
extern int bpfima_container_get_leaf_hash(const char *container_id, u8 *leaf_hash, u32 hash_size) __ksym;

/* ===== Merkle Tree kfuncs ===== */

/**
 * bpfima_merkle_get_root - Get the current Merkle tree root hash
 * @root_hash: Buffer to store the root hash (must be at least 32 bytes)
 * @hash_size: Size of hash buffer
 *
 * Returns: 0 on success, negative error code on failure
 */
extern int bpfima_merkle_get_root(u8 *root_hash, u32 hash_size) __ksym;
...
/**
 * bpfima_policy_update_min_file_size - Update minimum file size threshold
 * @namespace_id: Namespace/container identifier
...
/**
 * bpfima_policy_update_log_level - Update logging level for a namespace
 * @namespace_id: Namespace/container identifier
...
/**
 * bpfima_policy_get_changes_hash - Get hash of all policy changes
 * @namespace_id: Namespace/container identifier
