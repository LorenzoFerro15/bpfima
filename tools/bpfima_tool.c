...
static int cmd_load(const char *filename, bool daemon_mode)
{
    ...
    bpf_object__for_each_map(map, g_obj) {
        ...
        if (access(path, F_OK) == 0) {
            ...
            if (fd < 0) {
                fprintf(stderr, "Warning: can't reuse old map %s: Invalid file descriptor\nA new one will be created\n",  bpf_map__name(map));
                unlink(path);
                continue;
            }
            ...
            if (err) {
                fprintf(stderr, "Warning: can't reuse old map %s: %s\nA new one will be created\n", bpf_map__name(map), strerror(-err));
                close(fd);
                unlink(path);
                continue;
            }

            printf("  Reuse already pinned map: %s\n", bpf_map__name(map));
    ...
    bpf_object__for_each_map(map, g_obj)
    {
        ...
        // Do not try to pin an already pinned map, we reuse the old fd
